...
def test_agent_functionality():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("Agent is properly configured and can answer questions about the textbook.")
        elif response.status_code == 422:
            print("[ERROR] Request validation error - incorrect payload format")
            print(f"Response: {response.text}")
        else:
            print(f"[ERROR] Agent failed with status {response.status_code}")
            print(f"Response: {response.text}")
            
    except Exception as e:
        print(f"[ERROR] Error testing agent functionality: {e}")
