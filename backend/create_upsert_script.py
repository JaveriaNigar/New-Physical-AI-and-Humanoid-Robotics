"""
Script to re-index textbook content into Qdrant vector store.
Reads all markdown files from the physical-ai-book/docs directory and indexes them.
"""
import os
import sys
import uuid
...
def read_markdown_files(docs_dir: str) -> List[Dict[str, Any]]:
    ...
    for md_file in docs_path.rglob("*.md"):
        if md_file.name.startswith('_'):  # Skip files starting with underscore (like _category_.json)
            continue
...
def create_upsert_script():
    """
    Creates a script to upsert textbook content to Qdrant.
    """
    script_content = '''
import os
import sys
...
'''
    ...
    script_path = Path("C:/Users/M.R Computers/OneDrive/Desktop/hackathon2/backend/src/services/upsert_textbook.py")
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(script_content)
    
    print(f"Created upsert script at: {script_path}")

if __name__ == "__main__":
    print("Creating upsert script for textbook content...")
    create_upsert_script()
    print("Upsert script created successfully!")